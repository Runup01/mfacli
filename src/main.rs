...
fn print_clap_error(e: &clap::Error) {
    ...
    let msg = e.to_string();
    eprintln!();
    if msg.contains("unexpected argument") || msg.contains("unexpected value")
        || msg.contains("found argument") || msg.contains("found positional")
    {
        eprintln!("  {} 像是把值放错了位置，或漏了某个选项：", "提示".cyan().bold());
        eprintln!("    • 密码 / 密钥{}写在命令行 —— 会进 history 和进程列表，是泄露面。", "不要".red().bold());
        eprintln!("      密码用回车后的隐藏弹窗，或环境变量 {}；密钥用 {} 或省略后交互输入。",
            "MFA_PASSWORD".cyan(), "--secret".cyan());
        eprintln!("    • 看这个命令的完整用法：{}", "mfa <命令> --help".cyan());
    } else if msg.contains("required argument") || msg.contains("required arguments were not provided")
        || msg.contains("requires a subcommand")
    {
        eprintln!("  {} 少了必填参数，运行 {} 看需要填什么。",
            "提示".cyan().bold(), "mfa <命令> --help".cyan());
    } else if msg.contains("invalid value") || msg.contains("invalid subcommand") {
        eprintln!("  {} 值或子命令不对，运行 {} 看可选项。",
            "提示".cyan().bold(), "mfa --help".cyan());
    } else {
        eprintln!("  {} 不确定用法时，运行 {} 或 {}。",
            "提示".cyan().bold(), "mfa --help".cyan(), "mfa <命令> --help".cyan());
    }
}
...
fn print_app_error(e: Box<dyn std::error::Error>) {
    ...
    {
        eprintln!("  {} 用 {} 查看现有条目（名称区分大小写）。", "提示:".cyan(), "mfa list".cyan());
    } else if msg.contains("already exists") {
        eprintln!("  {} 名字已存在：改名用 {}，或先 {} 再添加。",
            "提示:".cyan(), "mfa edit <name> --rename <new>".cyan(), "mfa remove <name>".cyan());
    } else if msg.to_lowercase().contains("base32") {
        eprintln!("  {} 批量添加可直接导入文件：{}", "提示:".cyan(), "mfa import <文件>".cyan());
...
fn run(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    match cli.command {
        ...
        Some(Commands::Add { name, secret, issuer, algorithm, digits, period }) => {
            cmd_add(&name, secret.as_deref(), issuer.as_deref(), &algorithm, digits, period)
        }
        ...
        Some(Commands::Edit { name, rename, secret, issuer }) => cmd_edit(&name, rename.as_deref(), secret.as_deref(), issuer.as_deref()),
        ...
        Some(Commands::Config { pet, city, show_weather, show_bazi, show_pet }) => {
...
fn cmd_tui() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if entries.is_empty() {
        ...
        println!("  {}", "mfa add <name> --secret <SECRET> [--issuer <ISSUER>]".cyan());
...
fn cmd_init(encrypt: bool) -> Result<(), Box<dyn std::error::Error>> {
    if encrypt {
        ...
        let vault = storage::vault::Vault::load()?;
        vault.save()?;
        println!("{} Vault initialized (plain mode, file permissions 600)", "✓".green());
...
fn cmd_show(name: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  {} {}", "Issuer:".bold(), entry.issuer.as_deref().unwrap_or("-"));
...
fn display_width(s: &str) -> usize {
    s.chars().map(|c| {
        if c as u32 >= 0x1100 && (
            ...
            (c as u32 >= 0x30000 && c as u32 <= 0x3fffd)    // CJK Ext G+
        ) { 2 } else { 1 }
    }).sum()
}
...
fn cmd_list(limit: Option<usize>, show_all: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if entries.is_empty() {
        ...
        println!("    {} mfa add github --secret JBSWY3DPEHPK3PXP --issuer GitHub", "$".green());
    ...
    // ── Header: compact two-line info block ──
    println!();
    println!("  {}", "┌".cyan(), );
    // Line 1: time + weather
    ...
    // ── Sort entries by issuer then name ──
    ...
    sorted.sort_by(|a, b| {
        ...
        ia.to_lowercase().cmp(&ib.to_lowercase())
    ...
    let max_display = if show_all {
        ...
        limit.unwrap_or_else(|| {
            crossterm::terminal::size().map(|(_, h)| (h as usize).saturating_sub(10)).unwrap_or(20)
        })
    };
    ...
    let max_name_w = display_entries.iter()
        .map(|e| display_width(&truncate_str(&e.name, 36)))
        .max().unwrap_or(4).max(4);
    let max_issuer_w = display_entries.iter()
        .map(|e| {
    ...
    let name_col = max_name_w.clamp(4, 36);
    let issuer_col = max_issuer_w.clamp(6, 36);
    ...
    println!("  {}", "─".repeat(total_w).dimmed());
    if hidden_count > 0 {
        println!("  {} ({} more, use {} to see all)",
            "…".yellow(), hidden_count, "mfa list --all".cyan());
    }
    println!("  {}  {}  {}  {}",
        "tip".dimmed(),
        "mfa copy <name>".cyan(),
...
fn truncate_str(s: &str, max_w: usize) -> String {
    ...
    let truncated: String = s.chars().scan(0usize, |acc, c| {
        let cw = if (c as u32) > 0x7f { 2 } else { 1 };
        if *acc + cw > max_w - 1 { None } else { *acc += cw; Some(c) }
    }).collect();
    format!("{}…", truncated)
}
...
fn cmd_rename(old: &str, new: &str) -> Result<(), Box<dyn std::error::Error>> {
    let mut vault = storage::vault::Vault::load()?;
    vault.rename_entry(old, new)?;
    vault.save()?;
    println!("{} Renamed '{}' → '{}'", "✓".green(), old, new);
    Ok(())
}

fn cmd_edit(name: &str, rename: Option<&str>, secret: Option<&str>, issuer: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    let mut vault = storage::vault::Vault::load()?;
    ...
    if rename.is_none() && !want_secret_change && issuer.is_none() {
        ...
        println!("  {} {}", "Algorithm:".bold(), entry.algorithm);
        println!();
        println!("  Usage: mfa edit {} --rename <new> --secret --issuer <new>", name);
    ...
    if let Some(i) = issuer {
        entry.issuer = if i.is_empty() { None } else { Some(i.to_string()) };
    }
...
fn cmd_export(output: Option<&str>, format: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match format {
        "json" => {
            ...
            write_out(&data)?;
            if let Some(p) = output { println!("{} Exported {} entries as {} → {}", "✓".green(), entries.len(), "json".cyan(), p); }
            else { eprintln!("{} {} entries as {}", "✓".green(), entries.len(), "json".cyan()); }
        }
        "encrypted" => {
            let data = vault.export_encrypted()?;
            write_out(&data)?;
            if let Some(p) = output { println!("{} Exported {} entries as {} → {}", "✓".green(), entries.len(), "encrypted".cyan(), p); }
            else { eprintln!("{} {} entries as {}", "✓".green(), entries.len(), "encrypted".cyan()); }
        }
        _ => { // otpauth (default) — the universal, re-importable template
            let mut lines: Vec<String> = Vec::new();
            let mut skipped_steam = 0usize;
            for e in entries {
                if e.otp_type == "steam" { skipped_steam += 1; continue; }
                lines.push(e.to_otpauth_uri());
            }
            let data = if lines.is_empty() { String::new() } else { lines.join("\n") + "\n" };
            write_out(&data)?;
            let n = lines.len();
            if let Some(p) = output { println!("{} Exported {} entries as {} → {}", "✓".green(), n, "otpauth".cyan(), p); }
            else { eprintln!("{} {} entries as {}", "✓".green(), n, "otpauth".cyan()); }
            if skipped_steam > 0 {
                eprintln!("  {} {} Steam entries skipped (not in otpauth standard; use {} to keep them)",
                    "!".yellow(), skipped_steam, "--format json".cyan());
            }
        }
    }
    Ok(())
...
fn cmd_import(source: Option<&str>, path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for mut entry in entries {
        ...
        if vault.list_entries().iter().any(|e| e.name == entry.name) {
            ...
            let mut n = 2u32;
            while vault.list_entries().iter().any(|e| e.name == format!("{}_{}", base, n)) {
                n += 1;
            }
            entry.name = format!("{}_{}", base, n);
            renamed += 1;
            println!("  {} {} → {} (renamed, already exists)", "⚠".yellow(), original_name, entry.name);
        }
        match vault.add_entry(entry.clone()) {
            ...
            Err(e) => {
                skipped += 1;
                println!("  {} {:<16} {}", "✗".red(), entry.name, e.to_string().dimmed());
            }
        }
    }
    ...
    println!("  {} {} added, {} renamed, {} skipped (source: {})",
        "✓".green(), added, renamed, skipped, src);
    println!();
    Ok(())
}
...
fn cmd_config(
    ...
    if let Some(v) = show_pet {
        config.show_pet = v;
        println!("{} Pet display: {}", "✓".green(), if v { "on" } else { "off" });
        changed = true;
    }
    ...
    if !changed {
        ...
        println!("  {} {}", "City:".bold(), config.city.as_deref().unwrap_or("Auto (IP)").cyan());
        println!("  {} {}", "Weather:".bold(), if config.show_weather { "ON".green().to_string() } else { "OFF".red().to_string() });
        println!("  {} {}", "BaZi:".bold(), if config.show_bazi { "ON".green().to_string() } else { "OFF".red().to_string() });
        println!("  {} {}", "Pet Display:".bold(), if config.show_pet { "ON".green().to_string() } else { "OFF".red().to_string() });
        println!("  {} {}", "应用锁:".bold(), if encrypted { "已启用 (每次访问需密码)".green().to_string() } else { "未启用".yellow().to_string() });
        println!("  {} {}", "  提示:".dimmed(), if encrypted { "忘记密码无法找回；务必保留备份 json (mfa import 可恢复)".dimmed().to_string() } else { "启用: mfa lock    关闭: mfa unlock".dimmed().to_string() });
        println!();
...
fn cmd_lock(backup: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    if storage::vault::Vault::lock_status() == storage::vault::LockStatus::Locked {
        println!("{} 应用锁已启用，无需重复加锁。关闭请用 {}", "✓".green(), "mfa unlock".cyan());
        return Ok(());
    }
    ...
    println!("  {} 密码一旦忘记，加密的 vault.enc {}——没有后门、无法找回。",
        "!".red().bold(), "将永久无法打开".red().bold());
    println!("  所以下一步会先写一份{}，作为你的逃生绳。", "明文备份".cyan());
    println!();
    ...
    println!("  {} 明文备份已写入：{}", "✓".green(), backup_path.display().to_string().cyan());
    println!("  {} 这是{}，请转存到密码管理器 / 离线介质，{}放仓库或云盘同步目录。",
        "→".yellow(), "明文".red().bold(), "勿".red().bold());
    println!("  {} 只要它在，忘密码也能用 {} 恢复。",
        "→".yellow(), format!("mfa import {}", backup_path.display()).cyan());
    println!();

    // 2) 两次确认密码，必须完全一致
    let pw1 = rpassword::prompt_password("  设置锁密码: ")?;
    if pw1.is_empty() { return Err("密码不能为空".into()); }
    if pw1.len() < 8 {
        println!("  {} 密码少于 8 位，记错 / 被撞风险高。", "!".yellow());
        print!("  仍要继续请输入 yes：");
        let mut ans = String::new();
        std::io::BufRead::read_line(&mut std::io::stdin().lock(), &mut ans)?;
        if ans.trim() != "yes" { return Err("已取消加锁（备份已保留，可手动删除）".into()); }
    }
    let pw2 = rpassword::prompt_password("  再次输入确认: ")?;
    if pw1 != pw2 { return Err("两次密码不一致，未做任何更改（备份已保留）".into()); }
    ...
    println!("  {} 脚本 / 自动化可设环境变量 {} 免交互（该 shell 内等同常驻解锁，自行权衡）。",
        "→".yellow(), "MFA_PASSWORD".cyan());
...
fn cmd_unlock() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("  {} 关闭应用锁后，密钥将以{}存于 vault.json (仅权限 600 保护)。",
        "⚠".yellow().bold(), "明文".red().bold());
    println!("  共享 / 公共电脑请勿关闭。");
    ...
    println!("  数据已明文落盘 (权限 600)。如需重新保护：{}", "mfa lock".cyan());
    println!();
    Ok(())
}
