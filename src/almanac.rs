...
pub fn get_almanac() -> AlmanacInfo {
    ...
    let year_ganzhi = format!(
        "{}{}",
        HEAVENLY_STEMS[year_stem as usize], EARTHLY_BRANCHES[year_branch as usize]
    );

    // 月干支 (简化：以公历月份近似)
    let month_branch = (month + 1) % 12; // 寅月=1月 近似
    let month_stem = (year_stem * 2 + month) % 10;
    let month_ganzhi = format!(
        "{}{}",
        HEAVENLY_STEMS[month_stem as usize], EARTHLY_BRANCHES[month_branch as usize]
    );
    ...
    let day_ganzhi = format!(
        "{}{}",
        HEAVENLY_STEMS[day_stem as usize], EARTHLY_BRANCHES[day_branch as usize]
    );
...
pub fn format_header_line(info: &AlmanacInfo, show_bazi: bool) -> String {
    let mut parts = vec![format!(
        "{} {} {}",
        info.date_str, info.weekday, info.time_str
    )];

    if show_bazi {
        parts.push(format!(
            "{}年{}月{}日",
            info.year_ganzhi, info.month_ganzhi, info.day_ganzhi
        ));
        parts.push(format!(
            "{}日 宜:{} 忌:{}",
            info.officer_name, info.yi, info.ji
        ));
    }
